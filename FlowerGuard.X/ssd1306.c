...
#include "ssd1306.h"
#include "mcc_generated_files/i2c_driver.h"
...
static bit ssd1306_display_on;

void i2c_wait()
{
	while ((i2c_driver_isRead()) || (SSP1CON2 & 0x1F)); //Transmit is in progress
}

void i2c_send(const uint8_t* data, uint8_t length)
{
	static const uint8_t slave_addr = SSD1306_I2C_ADDRESS << 1;
