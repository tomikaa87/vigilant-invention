...
void SSD1306::sendCommand(uint8_t code)
{
    ...
    i2c_write_blocking(i2c0, SSD1306_I2C_ADDRESS, data, sizeof(data), false);
}
...
void SSD1306::sendData(uint8_t data, const uint8_t bitShift, const bool invert)
{
    ...
    i2c_write_blocking(i2c0, SSD1306_I2C_ADDRESS, buf, sizeof(buf), false);
}

void SSD1306::sendData(const uint8_t* data, const size_t length, const uint8_t bitShift, const bool invert)
{
    // No bitshift and invert support for now for extra speed

    // Write DC flag without sending STOP
    const uint8_t flag = SSD1306_I2C_DC_FLAG;
    if (i2c_write_blocking(i2c0, SSD1306_I2C_ADDRESS, &flag, sizeof(flag), true /* nostop */) == PICO_ERROR_GENERIC) {
        return;
    }

    // Write the whole data buffer
    i2c_write_blocking(i2c0, SSD1306_I2C_ADDRESS, data, length, false);
}
