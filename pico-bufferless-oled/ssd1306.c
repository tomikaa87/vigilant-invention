...
static bool ssd1306_display_on;
// static int i2c_file = 0;

static ssd1306_i2c_send_fn i2c_send_fn = NULL;

void i2c_wait()
{
}

void i2c_send(const uint8_t* data, uint8_t length)
{
    if (i2c_send_fn) {
        i2c_send_fn(data, length);
    }

    // int written = write(i2c_file, data, length);
    // if (written != length) {
    //     printf("I2C write failed: i2c_file=%d, length=%d, written=%d\n", i2c_file, length, written);
    // }
}

void ssd1306_set_i2c_send_fn(ssd1306_i2c_send_fn f)
{
    i2c_send_fn = f;
}
...
void ssd1306_init()
{
    ...
    // printf("Setting I2C slave address: %02x\n", SSD1306_I2C_ADDRESS);
    // if (ioctl(i2c_file, I2C_SLAVE, SSD1306_I2C_ADDRESS) < 0) {
...
void ssd1306_scroll(ssd1306_scroll_t scroll, uint8_t start, uint8_t stop)
{
    ...
    case SSD1306_SCROLL_DIAG_LEFT:
        ...
        ssd1306_send_command(0);
        ssd1306_send_command((ssd1306_cmd_t)SSD1306_LCDHEIGHT);
