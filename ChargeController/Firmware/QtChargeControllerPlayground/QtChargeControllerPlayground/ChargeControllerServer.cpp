...
ChargeControllerServer::ChargeControllerServer(std::unique_ptr<IServerAdapter>&& serverAdapter)
    : m_serverAdapter{ std::move(serverAdapter) }
{
...
ChargeControllerServer::~ChargeControllerServer()
{
}

void ChargeControllerServer::onSwitch(SwitchHandler&& handler)
{
    m_switchHandler = std::move(handler);
}
...
void ChargeControllerServer::switchController(const uint16_t endpointId, const nlohmann::json& req)
{
    ...
    if (!m_switchHandler)
    {
        sendErrorResponseToClient(endpointId, req, ErrorCode::InternalError);
        return;
    }

    m_switchHandler(req["param"]["on"] != 0);
